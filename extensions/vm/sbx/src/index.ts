...
export default function (pi: ExtensionAPI, probe?: SideProbe) {
  if (!requireSide("host", pi, "sbx", probe)) return;
  ...
  // `user_bash` fires for both `!` and `!!`. `!` is routed into the sandbox
  ...
  pi.on("user_bash", async (event, ctx) => {
    ...
    return { operations: createBashOperations(boundRun(active.info)) };
  });
