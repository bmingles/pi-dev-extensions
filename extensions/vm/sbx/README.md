...
[`extensions/host/devcontainer`](../../host/devcontainer) — override the built-in
...
`extensions/host/devcontainer`: the two backends' lifecycle, path model, and
...
**Side: host.** This extension runs on the host — `vm/` names what it routes
*into* (an isolated sandbox/VM), not where this process runs. Loaded on a
container side (`/.dockerenv` present), `requireSide` (from
`pi-extension-core`) refuses before registering anything — a `session_start`
notification names the extension and the mismatch.
...
- Reads and writes therefore reflect the **sandbox's** filesystem. Because
  ...
  `extensions/host/devcontainer`, there is no `toContainerPath`/
...
- Adds two **new** tools: `read_host`, the escape hatch that reads the
  **host** filesystem, and `list_host_docs`, an unprompted listing
  ...
  [`pi-extension-host-read-core`](../../shared/host-read-core) package (Phase 30).
...
This is `extensions/vm/sbx`'s one structural simplification over
`extensions/host/devcontainer` — there is no `paths.ts` path-mapping module
...
[`pi-extension-host-read-core`](../../shared/host-read-core) package (Phase 30), not
in this extension — see that package's README for the shared machinery
itself, and `extensions/host/devcontainer/README.md` for a fuller writeup of
the tools' user-facing behavior (identical here).

### How the single-mount model simplifies the barrier vs. devcontainer's

`extensions/host/devcontainer`'s `getMounts` reads the container's mounts through
`@devc-tools/core` — a dynamic, N-entry table (bind + volume mounts) that can
change
between sandboxed runs. `extensions/vm/sbx`'s `getMounts` (in `src/tools.ts`)
...
home directory itself — e.g. running `pi -e .../extensions/vm/sbx` from `~`
...
- **Cross-sandbox / poison-then-teardown.** Same gap
  `extensions/host/devcontainer` already documents as out of scope: a removed
...
npm install        # install pi (types) + typebox + typescript + @types/node,
                    # and resolve the file:../../shared/host-read-core dependency
...
- `cd <project> && pi -e extensions/vm/sbx`, then have the agent
...
- Create a sandbox for a workspace outside this extension (e.g. plain `sbx
  ...
  extensions/vm/sbx` from that same `<dir>` — confirm the picker offers
