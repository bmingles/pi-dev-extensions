 ...
 *
 * Runs on the HOST — `vm/` names what it routes *into* (a per-session micro-VM), not where
 * this process runs. `requireSide` refuses to load this on a container side.
 */
...
export default function (pi: ExtensionAPI, probe?: SideProbe) {
  if (!requireSide("host", pi, "gondolin", probe)) return;
  ...
  // `user_bash` fires for both `!` and `!!`. `!` is routed into the VM like
  // the LLM's own `bash` tool — the two should behave identically. `!!`
