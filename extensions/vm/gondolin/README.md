...
*into* (a local micro-VM), not where this process runs. Loaded on a container
side (`/.dockerenv` present), `requireSide` (from `pi-extension-core`)
refuses before registering anything — a `session_start` notification names
the extension and the mismatch.
