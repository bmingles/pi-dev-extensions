...
## ⚠️ `herdr_devc_*` (here) vs. `devcontainer_herdr_*` (in `extensions/host/devcontainer`)

Two mirror-image tool families now exist in this repo, and they serve
**opposite topologies**:

| | `herdr_devc_*` (this extension) | `devcontainer_herdr_*` ([`extensions/host/devcontainer`](../../host/devcontainer)) |
| --- | --- | --- |
...
all — `requireSide` (from `pi-extension-core`) refuses before registering
anything, notifying at `session_start` instead. This closes the defect that
motivated the split: the `NOT_A_MOUNT` guard above used to be gated on
`ResolveDeps.isContainer` alone, so a host load skipped it entirely and
`herdr_devc_worktree_path` reported `mounted: true` for any path without
checking anything.

## Shared plumbing lives in `extensions/shared/herdr-core`
...
and `expandTilde` —
live in [`pi-extension-herdr-core`](../../shared/herdr-core), a private library package
...
[`herdr-core`](../../shared/herdr-core), and their tests moved with them.
